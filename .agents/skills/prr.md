...
- `simple` — outage cost < $0k/min OR internal tool — 4-8h, 1 sessão
- `early` — outage cost $0k-100k/min OR customer-facing — semanas, SRE no design
- `platform` — outage cost > $000k/min OR built on Frameworks/SRE Platform — PRR é confirmação
...
```bash
SERVICE=$(echo "$ARGUMENTS" | grep -oE -- '--service [^ ]+' | awk '{print $1}')
FEATURE=$(echo "$ARGUMENTS" | grep -oE -- '--feature "[^"]+"' | sed 's/--feature //; s/^"//; s/"$//')
ENGAGEMENT=$(echo "$ARGUMENTS" | grep -oE -- '--engagement [^ ]+' | awk '{print $1}')
REVIEWER=$(echo "$ARGUMENTS" | grep -oE -- '--reviewer [^ ]+' | awk '{print $1}')
OUTAGE_COST=$(echo "$ARGUMENTS" | grep -oE -- '--outage-cost [^ ]+' | awk '{print $1}')
OUTPUT_PATH=$(echo "$ARGUMENTS" | grep -oE -- '--output [^ ]+' | awk '{print $1}')
...
> options:
> - "< $0k/min OR internal tool → Simple PRR (4-8h, 1 sessão)"
> - "$0k-100k/min OR customer-facing → Early Engagement (semanas, SRE no design)"
> - "> $000k/min OR built on platform → Frameworks/Platform (PRR é confirmação)"
